...
import { MoreHorizontal, Play, Trophy } from "lucide-react"
...
// Function to generate a deterministic pattern (dots/circles)
const generatePatternOverlay = (text: string) => {
    // Simple check to decide pattern type usually based on string length or char codes
...
export function QuizCard({ id, title, description, category }: QuizCardProps) {
    const gradient = generateGradient(title + category);
        ...
        <div className="group relative flex flex-col bg-[#131f24] border-2 border-[#37464f] rounded-2xl overflow-hidden hover:border-[#58cc02] hover:-translate-y-1 transition-all duration-300 shadow-lg hover:shadow-[0_10px_20px_rgba(0,0,0,0.3)]">
            ...
            <div className="p-5 flex-1 flex flex-col">
                ...
                <p className="text-[#afafaf] text-sm leading-relaxed mb-6 line-clamp-3 flex-1 font-medium">
                    {description || "No description available for this quiz."}
                </p>
                ...
                <div className="flex items-center justify-between border-t border-[#37464f] pt-4 mt-auto">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-bold text-[#afafaf]">Quiz</span>
                    </div>

                    <Link href={`/quiz/${id}`} className="flex items-center gap-2 text-white bg-[#37464f] hover:bg-[#58cc02] hover:text-[#131f24] px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300">
